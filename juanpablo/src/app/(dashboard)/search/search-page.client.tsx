...
const isDeferredFilterKey = (key: keyof FilterState): boolean => {
  return key === "query" || key === "minPrice" || key === "maxPrice";
};
...
const hasInvalidPriceRangeForFilters = (filters: FilterState): boolean => {
  ...
  if (minPriceRaw.length === 0 || maxPriceRaw.length === 0) {
    return false;
  }
...
const requestSaveSearchFilter = async (
  filters: FilterState,
...
export function SearchPageClient({
  ...
  const navigateToState = useCallback(
    (nextFilters: FilterState, nextPage: number, nextPageSize: SearchPageSize) => {
      if (hasInvalidPriceRangeForFilters(nextFilters)) {
        return;
      }

      const nextQueryString = buildSearchParams(nextFilters, nextPage, nextPageSize).toString();

      if (nextQueryString === committedQueryString) {
        return;
      }
  ...
  const setFilterValue = useCallback(
    <K extends keyof FilterState>(key: K, value: FilterState[K]) => {
      ...
      const nextFilters = updateFilters((previous) => {
        const nextFilters = {
          ...previous,
          [key]: value,
        };
  ...
  const setSort = useCallback((value: SearchSort) => {
    setSaveSearchFilterMessage("");
    const nextFilters = updateFilters((previous) => {
      const nextFilters = {
        ...previous,
